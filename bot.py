...
TARGET_CHANNEL_ID = 1536880493730070629

# 2. أيدي رتبة الإدارة أو المشرفين
ADMIN_ROLE_ID = 1521795813171531836
...
async def on_message(message):
  ...
  is_admin = message.author.guild_permissions.administrator or any(
    ...
    # تسجيل الخروج (خ)
      ...
      if user_id in active_sessions:
        ...
        await message.channel.send(
            f"{message.author.mention}، أنت لم تقم بتسجيل الدخول بـ 'د' مسبقاً!"
        )
      return
  ...
  if cmd == "س":
    ...
    embed = discord.Embed(
        title="ساعات العمل",
        description=(
            f"مجموع ساعات {target_user.mention} هي:"
  ...
  if is_admin and message.mentions:
    ...
    if cmd == "زياده" and len(parts) >= 2:
      try:
        ...
        user_hours[target_user.id] = (
            user_hours.get(target_user.id, 0) + added_mins
        )
        await message.channel.send(
            f"تم زيادة {parts[1]} ساعة لـ {target_user.mention} بنجاح."
        )
      except ValueError:
        await message.channel.send(
            "الرجاء كتابة الرقم بشكل صحيح (مثال: زياده 2.00 @شخص)"
        )
      return
    ...
    # نقص الساعات
      ...
      try:
        ...
        await message.channel.send(
            f"تم نقص {parts[1]} ساعة من {target_user.mention} بنجاح."
        )
      except ValueError:
        await message.channel.send(
            "الرجاء كتابة الرقم بشكل صحيح (مثال: نقص 2.00 @شخص)"
        )
      return
    ...
    # إلغاء الدخول
      ...
      if target_user.id in active_sessions:
        ...
        await message.channel.send(
            f"تم إلغاء تسجيل دخول {target_user.mention} ولم يُحتسب له أي وقت."
        )
      else:
        await message.channel.send(
            f"العضو {target_user.mention} ليس مسجل دخول أصلاً."
        )
      return
    ...
    if before.channel is not None and after.channel is None:
      ...
      try:
        embed = discord.Embed(
            ...
            description=(
                "🕹️ تم رصُد خروجك من الرومات الصوتية. لديك 10 دقائق للعودة قبل"
                " إلغاء تسجيل الدخول."
            ),
            color=0xf1c40f,
        )
        embed.add_field(
            name="الشرط المفقود", value="الرومات الصوتية", inline=False
        )
        embed.add_field(name="المهلة", value="10 دقائق", inline=False)
        embed.set_footer(
            text=f"الوقت والتاريخ: {datetime.now().strftime('%H:%M %Y/%m/%d')}"
        )
      ...
      if active_sessions[member.id].get("warned", False):
        ...
        try:
          embed = discord.Embed(
              title="تم رصد عودتك",
              description=(
                  "✅ تم رصد عودتك الى الرومات الصوتية والسيرفر. تم إلغاء تنبيه"
                  " فقدان الشرط."
              ),
              color=0x2ecc71,
          )
          embed.add_field(
              name="الشرط المستعاد",
              value="الرومات الصوتية والسيرفر",
              inline=False,
          )
          embed.set_footer(
              text=f"الوقت والتاريخ: {datetime.now().strftime('%H:%M %Y/%m/%d')}"
